jQuery(function () {
    ...
    let waist_size = $("#input_waist").val()
    let legs_size = $("#input_legs_size").val()
    let wrist_size = $("#input_wrist").val()
    let custom_radio_btn = $("#custom_calculation").is(':checked')
    let correct_password = $("#input_password").val()
    let correct_password_confirmation = $("#input_password_confirmation").val()
    let data = {}
    ...
    updateRequiredFields(automatic_radio_btn)
    ...
    $("#input_caloric_requirement").on('input', function () {
        ...
        if (custom_radio_btn) {
            caloric_requirement && caloric_requirement > 0 ? $(this).removeClass('is-invalid').addClass('is-valid') : $(this).removeClass('is-valid').addClass('is-invalid')
        }
    })
    ...
    $("#update_user_btn").on('click', function () {
        ...
        if (automatic_radio_btn) {
            console.log('valor de automatic radio button:', automatic_radio_btn)
        ...
        $("#update_user_btn").attr('hidden', true);
    ...
    $("#update_nutritionist_btn").on('click', function () {
        if (!name) {
            $("#input_name").trigger('focus').removeClass('is-valid').addClass('is-invalid')
            return;
        }
        if (!last_name) {
            $("#input_last_name").trigger('focus').removeClass('is-valid').addClass('is-invalid')
            return;
        }
        if (!email) {
            $("#input_email").trigger('focus').removeClass('is-valid').addClass('is-invalid')
            return;
        }
        if (password && password.length < 8) {
            $("#input_password").trigger('focus').removeClass('is-valid').addClass('is-invalid')
            return;
        }
        if (password_confirmation && password_confirmation.length < 8) {
            $("#input_password_confirmation").trigger('focus').removeClass('is-valid').addClass('is-invalid')
            return;
        }
        if (identificator && !validateIdentificator(identificator)) {
            $("#input_identificator").trigger('focus').removeClass('is-valid').addClass('is-invalid')
            return;
        }
        data = {
            name,
        ...
        const user_update_route = $("#update_nutritionist_route").val();
        $("#update_nutritionist_btn").attr('hidden', true)
        $("#loader").attr('hidden', false)
