jQuery(function () {
    ...
    const food_table = $("#food_table").DataTable({
        "pageLength": 5,
    ...
    $("#food_amount").on('input', function () {
        food_amount = this.value
        food_name_validation == 1 && food_amount && food_amount > 0
            ? $("#add_food").attr('disabled', false) : $("#add_food").attr('disabled', true);
    })
    $("#food_amount").on('keyup', function (e) {
        /* add food to menu if user press Enter key */
        if (e.key === 'Enter') {
            addFood()
        }
    })
    ...
    */
    ...
    $("#menuComponentFoodWeightEdited").on('input', function () {
        let amount = this.value
        !amount || amount <= 0 ? $("#update_component_btn").attr('disabled', true)
